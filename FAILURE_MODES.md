...
| Redis unavailable | Readiness fails and sessions, rate limiting, cooldowns, and caches are disrupted. Standard production and Saltbox Compose enable AOF, but AOF is not a substitute for backup. | Restore Redis; users may need to sign in again and transient state may be lost. Durable domain state remains in PostgreSQL. |
...
| Multiple bot processes | Each process would run its own scheduler and interval maintenance, duplicating work that is not consistently process-owned. | Checked-in deployments run one bot process. Do not scale or add sharding until scheduler ownership is designed and tested. |
