-- 0001_baseline
--
-- The schema as it stood when versioned migrations were introduced, lifted
-- verbatim from the old db/migrate.ts. Every statement is IF NOT EXISTS, which
-- is what makes it safe to apply to the already-populated production database:
-- there it is a no-op that simply gets recorded as applied, while on a fresh
-- database it builds everything.
--
-- Do not edit this file. The runner checksums applied migrations and will
-- refuse to continue if one changes. Add a new numbered file instead.
